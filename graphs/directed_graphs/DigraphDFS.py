...
class DigraphDFS:
    ...
    DFS is originally a digraph algorithm.
    """
    CONNECTED     = True
...
class TestsDigraphDFS(unittest.TestCase):
    ...
    def test_006_init__multisource(self):
        ...
        dfs = DigraphDFS(self.G, multisource)
        self.assertEqual( dfs._S, multisource )

        self.assertTrue( dfs._marked[ self.one_from_source  ] )
        self.assertTrue( dfs._marked[ self.two_from_source  ] )
        self.assertTrue( dfs._marked[ self.strictly_inbound ] )
        self.assertTrue( dfs._marked[ self.double_linked    ] )
        self.assertTrue( dfs._marked[ self.strictly_outbound] )
        self.assertTrue( dfs._marked[ self.not_connected    ] )
    ...
    def test_101_dfs(self):
        ...
        self.assertTrue ( self.dfs._marked[ self.strictly_inbound ] )
        self.assertTrue ( self.dfs._marked[ self.double_linked    ] )
        self.assertFalse( self.dfs._marked[ self.strictly_outbound] )
        self.assertFalse( self.dfs._marked[ self.not_connected    ] )
        
    def test_102_dfs__source_is_strictly_inbound_vertex(self):
        self.assertTrue     ( self.dfs._marked [self.strictly_inbound] )
        self.assertIsNotNone( self.dfs._edge_to[self.strictly_inbound] )

        dfs = DigraphDFS(self.G, self.strictly_inbound)
        self.assertTrue( any(dfs._marked[1:]) )         # except itself 

    def test_103_dfs__source_is_strictly_outbound_vertex(self):
        self.assertFalse ( self.dfs._marked [self.strictly_outbound] )
        self.assertIsNone( self.dfs._edge_to[self.strictly_outbound] )

        dfs = DigraphDFS(self.G, self.strictly_outbound)
    ...
    def test_201_is_reachable(self):
        ...
        self.assertTrue ( self.dfs.is_reachable( self.strictly_inbound ) )
        self.assertTrue ( self.dfs.is_reachable( self.double_linked    ) )
        self.assertFalse( self.dfs.is_reachable( self.strictly_outbound) )
